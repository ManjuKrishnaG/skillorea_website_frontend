...
const WellnessEnquiry = () => {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/wellness-enquiries/${deleteId}`
      );
  ...
  useEffect(() => {
    const fetchEnquiries = async () => {
      try {
        const res = await axios.get(
          `${API_BASE_URL}/wellness-enquiries/?page=${currentPage}&limit=${limit}`
        );
        const data = res.data;
        console.log("✅ API Response:", data);
        ...
        <Col md={10} className="main-content flex-grow-1">
          ...
          <div className="header">
            {/* <img
              ...
              height="40"
            /> */}
            <h2 style={{ marginTop: "-40px" }}> Wellness Enquiry Section</h2>
          </div>
            ...
            <Modal.Body>
              {selectedEnquiry && (
                <div>
                  ...
                  <p>
                    <strong>Mobile:</strong> {selectedEnquiry.mobile}
                  </p>
                  <p>
